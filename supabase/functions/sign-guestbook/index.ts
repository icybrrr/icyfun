// icybearOS guestbook -- the only write path.
//
// The anon key ships in public JavaScript, so nothing here may rely on the
// client having behaved. Every rule is enforced again, from scratch, on input
// that is assumed hostile.
//
// Secrets (Supabase -> Project Settings -> Edge Functions -> Secrets):
//   TELEGRAM_BOT_TOKEN  TELEGRAM_CHAT_ID  IP_SALT
...
// Submitted as an index, never as text, so this field cannot carry a payload.
const STAMPS: string[] = ["🎀", "🐻", "🐻‍❄️", "🐼", "🧸", "😇", "🪽", "✨", "💜", "🧁", "🍓", "🧋", "🥛", "🍄", "🌸", "🦄", "🦋", "🌈", "😹", "🫶", "⭐", "☁️", "❄️", "👑", "🌙", "🕊️", "💌"];
...
Deno.serve(async (req) => {
  ...
  // ---- stamp: an index into our own list, so it cannot be arbitrary text
  const idx = body.stamp;
  if (typeof idx !== "number" || !Number.isInteger(idx) ||
      idx < 0 || idx >= STAMPS.length) {
    return reply({ error: "stamp" }, 400, origin);
  }
  const stamp = STAMPS[idx];

  // ---- name: normalise BEFORE every check, or each one can be walked past
  ...
  // "...", "@@@" and fourteen spaces all satisfy the allowlist otherwise
  ...
  // ---- rate limit + insert, atomically, in one transaction
  const ipHash = await sha256(clientIp(req) + (Deno.env.get("IP_SALT") ?? ""));

  const url = Deno.env.get("SUPABASE_URL")!;
  const key = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
  const rpc = await fetch(url + "/rest/v1/rpc/gb_sign", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      apikey: key,
    ...
    body: JSON.stringify({ p_ip_hash: ipHash, p_name: name, p_stamp: stamp }),
  });
  ...
  // ---- ping the operator.
  //
  // AWAITED, deliberately. This was fire-and-forget with a comment claiming it
  // "never blocks the signature" -- which was true, and also meant it never
  // ran: Deno Deploy tears the isolate down as soon as the response returns, so
  // a floating promise is killed before it reaches Telegram.
  //
  // Bounded by a 3s abort so a slow Telegram still cannot hang a signature, and
  // every failure is logged rather than swallowed, so a broken token shows up in
  // the Edge Function logs instead of silently doing nothing.
  const token = Deno.env.get("TELEGRAM_BOT_TOKEN");
  const chat = Deno.env.get("TELEGRAM_CHAT_ID");
  if (!token || !chat) {
    console.error("telegram: secret missing", { token: !!token, chat: !!chat });
  } else {
    ...
    try {
      const tg = await fetch("https://api.telegram.org/bot" + token + "/sendMessage", {
        ...
        body: JSON.stringify({
          chat_id: chat,
          text: "guestbook " + stamp + "  " + name + "\nid " + out.id,
        }),
        signal: ctl.signal,
      });
      if (!tg.ok) console.error("telegram: http " + tg.status, await tg.text());
    } catch (e) {
      /* NAME ONLY, never String(e). Deno puts the full request URL inside a
         fetch rejection message -- "error sending request for url
         (https://api.telegram.org/bot<TOKEN>/sendMessage)" -- so any DNS or TLS
         failure was printing the bot token in plaintext into the Edge Function
         logs. The comment at the top of save/index.ts already says the Telegram
         token taught us how quietly a secret reaches a log; this was the same
         leak, in the handler written to make failures visible. */
  ...
  return reply({ ok: true, id: out.id, name, stamp }, 200, origin);
});
